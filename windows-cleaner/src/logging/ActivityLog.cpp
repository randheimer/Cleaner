...
void ActivityLog::Initialize(const std::wstring& logPath) {
    ...
    if (logFile.is_open()) {
        ...
        logFile << L"[" << GetTimestamp() << L"] "
                << L"[INFO] "
                << L"ActivityLog initialized" << std::endl;
        logFile.flush();
    }
}

void ActivityLog::Log(LogLevel level, const std::wstring& message) {
    std::lock_guard<std::mutex> lock(logMutex);
...
void ActivityLog::LogToConsole(LogLevel level, const std::wstring& message) {
...
std::wstring ActivityLog::GetTimestamp() {
...
std::wstring ActivityLog::LogLevelToString(LogLevel level) {
    switch (level) {
