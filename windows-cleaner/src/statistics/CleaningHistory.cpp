...
CleaningHistory& CleaningHistory::GetInstance() {
    static CleaningHistory instance;
    return instance;
}

void CleaningHistory::Initialize(const std::wstring& dbPath) {
    databasePath = dbPath;
    LoadDatabase();
}

void CleaningHistory::LoadDatabase() {
    sessions.clear();
    
    if (!fs::exists(databasePath)) {
        ActivityLog::GetInstance().Log(LogLevel::INFO, L"No existing stats database found, creating new one");
        return;
    }
    
    std::ifstream file(databasePath);
    if (!file.is_open()) {
        ActivityLog::GetInstance().Log(LogLevel::WARNING, L"Could not open stats database for reading");
        return;
    }
    ...
    file.close();
    ActivityLog::GetInstance().Log(LogLevel::INFO, 
        L"Loaded " + std::to_wstring(sessions.size()) + L" sessions from database");
}

void CleaningHistory::SaveDatabase() {
    std::ofstream file(databasePath, std::ios::trunc);
    if (!file.is_open()) {
        ActivityLog::GetInstance().Log(LogLevel::ERR, L"Could not open stats database for writing");
        return;
    }
    ...
    file.close();
    ActivityLog::GetInstance().Log(LogLevel::INFO, L"Stats database saved successfully");
}

void CleaningHistory::SaveSession(const CleaningSession& session) {
    sessions.push_back(session);
    SaveDatabase();
    ActivityLog::GetInstance().Log(LogLevel::INFO, L"New cleaning session saved to database");
}

AggregatedStats CleaningHistory::AggregateSessionsInTimeRange(time_t startTime, time_t endTime) {
...
AggregatedStats CleaningHistory::AggregateAllSessions() {
    AggregatedStats stats;
